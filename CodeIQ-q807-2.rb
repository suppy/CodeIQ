...
# フィードバックの内容を元に修正して再提出
## suppy193様の解答は残念ながら不正解でした。
## 0～9の出現回数が1回以下のものを求められているようで、1度も使われないものが存在します。
## 「一度ずつ」使っているものを探していただければと思います。
...
symmetry.permutation(2) do |pair1, pair2|
	...
	# 重複がなかった場合
	# かつ、すべての数字を使っている場合（2014.04.03追加）
	if num_array.uniq.length == num_array.length and num_array.uniq.length == 10
		# print "#{pair1}, #{pair2}\n"
