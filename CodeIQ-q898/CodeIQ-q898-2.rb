...
(0...20).to_a.combination(10).to_a.each do |pattern_color1|
	# 左上をスタート地点に設定
	check_point = [pattern_color1[0]]
	# 指定した分割方法のパターンになっていなければ次のパターンの処理へ
	next if check_neighbor(pattern_color1, pattern_color1[0], check_point) == false

	# 逆の色の処理
	# 逆の色のパターンを設定する
	pattern_color2 = (0...20).to_a - pattern_color1
	# パターンの先頭をスタート地点に設定
	check_point = [pattern_color2[0]]
	# 指定した分割方法のパターンになっていればカウントする
	num += 1 if check_neighbor(pattern_color2, pattern_color2[0], check_point) == true
end
p num
