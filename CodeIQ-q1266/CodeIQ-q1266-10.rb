...
answer = [3270540, 7914270, 8862945, 13506675]
...
pattern = 15
#p display(pattern)
p reverse(pattern, 0).to_s(2)
pattern = 15
display(pattern)
p swap(pattern, 0, 12).to_s(2)
display(swap(pattern, 0, 12))


def rotate180(pattern)
  rotate_pattern = 0
  (@width*@height).times do |i|
    rotate_pattern += 1 << (@width*@height - 1 - i) if pattern % 2 == 1
    pattern /= 2
  end
...
def flip(pattern)
  ...
  (0...@height).each do |y|
    (0...@width/2).each do |x|
      pos1 = x + y * @width
      pos2 = @width-1 - x + y * @width
#      print "#{pos1},#{pos2}\n"
#      print "#{(pattern >> pos1) % 2} #{((pattern >> pos2) % 2) }\n"
#      print "#{(((pattern >> pos1) % 2) << pos2)} #{((pattern >> pos2) % 2) << pos1}\n"  
      flip_pattern += (((pattern >> pos1) % 2) << pos2) + (((pattern >> pos2) % 2) << pos1)
      
 #     p flip_pattern.to_s(2)
    end
  end
  return flip_pattern
end
...
def invert(pattern)
  invert_pattern = (2 ** (@width*@height) - 1) ^ pattern
...
p @goal_patterns
display(@goal_patterns[0])
display(@goal_patterns[1])
display(@goal_patterns[2])
display(@goal_patterns[3])

def goal?(pattern)
  return true if @goal_patterns.include?(pattern)
  return false
end
...
#  print "(#{x1}, #{y1}) -> "
  if (pattern >> pos) % 2 == (goal_pattern >> pos) % 2
    return [], 0
  end
...
(0...@width*@height).each do |pos|
  print "#{pos} #{find_near_opposite_color(pattern, pos, goal_pattern)}\n"
end


@memo = {}
def search(pattern, goal_pattern, times)
  #puts "search"
  #return times > @min_length
#  if @memo.has_key?(pattern) && times > @min_length
#    return
#  else
#    @memo[pattern] = times
#  end
  if pattern == goal_pattern
    #print "goal #{times}\n"
    #@min_length = [@min_length, times].min
#    if @memo.has_key?(pattern)
#      @memo[pattern] = [@memo[pattern], times].min
#    else
      @memo[pattern] = 0
#    end
    return
  ...
  (0...@width*@height).each do |pos|
    next if (pattern >> pos) % 2 == (goal_pattern >> pos) % 2
    poses, length = find_near_opposite_color(pattern, pos, goal_pattern)
    #print "#{pos} -> #{poses}, #{length}\n"
    poses.each do |pos2|
      next_pattern = swap(pattern, pos, pos2)
      #p next_pattern
      #display(next_pattern)
      if !@memo.has_key?(next_pattern)
        search(next_pattern, goal_pattern, times + length)
      end
      if !@memo.has_key?(pattern)
...
sets_length = (0...@width*@height).to_a.combination(@width*@height/2).to_a.length

@memo = {}
max_length = 0
max_pattern = []
(0...@width*@height).to_a.combination(@width*@height/2) do |set|
  pattern = 0
  set.each do |i|
    pattern += 1 << i
  end
  ...
  #next if checked_pattern.include?(pattern)
  #checked_pattern += symmetry(pattern)
  #display(pattern)

  #@memo = {}
  @min_length = Float::INFINITY
  @goal_patterns.each do |goal_pattern|
    next if match_goal(pattern, goal_pattern) < @width*@height/4/2
    search(pattern, goal_pattern, 0)
  end
#end
  #p @memo
